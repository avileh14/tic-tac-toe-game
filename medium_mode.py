...
def check_win(player):
    ...
    if board[0][0] == player and board[1][1] == player and board[2][2] == player and board[3][3] == player:
        draw_diagonal_winning_line_LR(player)
        pygame.display.update()
        sleep(1)
        if player == 1:
            screen.fill(BG_COLOR)
    ...
    if board[0][3] == player and board[1][2] == player and board[2][1] == player and board[3][0] == player:
        draw_diagonal_winning_line_RL(player)
        pygame.display.update()
        sleep(1)
        if player == 1:
            screen.fill(BG_COLOR)
...
def draw_vertical_winning_line(col, player):
    if player == 1:
        pygame.draw.line(screen, CIRCLE_COLOR, (col * SQUARE_SIZE + SQUARE_SIZE // 2, 0),
                         (col * SQUARE_SIZE + SQUARE_SIZE // 2, HEIGHT), WIN_LINE_WIDTH)
    else:
        pygame.draw.line(screen, CROSS_COLOR, (col * SQUARE_SIZE + SQUARE_SIZE // 2, 0),
                         (col * SQUARE_SIZE + SQUARE_SIZE // 2, HEIGHT), WIN_LINE_WIDTH)

def draw_horizontal_winning_line(row, player):
    if player == 1:
        pygame.draw.line(screen, CIRCLE_COLOR, (0, row * SQUARE_SIZE + SQUARE_SIZE // 2),
                         (WIDTH, row * SQUARE_SIZE + SQUARE_SIZE // 2), WIN_LINE_WIDTH)
    else:
        pygame.draw.line(screen, CROSS_COLOR, (0, row * SQUARE_SIZE + SQUARE_SIZE // 2),
                         (WIDTH, row * SQUARE_SIZE + SQUARE_SIZE // 2), WIN_LINE_WIDTH)

def draw_diagonal_winning_line_LR(player):
    if player == 1:
        color = CIRCLE_COLOR
    elif player == 2:
        color = CROSS_COLOR
        ...
        return
    pygame.draw.line(screen, color, (0, 0), (WIDTH, HEIGHT), WIN_LINE_WIDTH)

def draw_diagonal_winning_line_RL(player):
    if player == 1:
        color = CIRCLE_COLOR
    elif player == 2:
        color = CROSS_COLOR
        ...
        return
    pygame.draw.line(screen, color, (0, HEIGHT), (WIDTH, 0), WIN_LINE_WIDTH)
